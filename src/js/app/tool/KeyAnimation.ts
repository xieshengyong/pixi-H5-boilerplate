...
// new fn();
interface KeyAnimationValue {
    goto: (n: number) => void;
    next: () => void;
    prev: () => void;
    fromTo: (from: number, to: number, loop?: any, callback?: () => void) => void;
    toFrom: (to: number, from: number, loop?: any, callback?: () => void) => void;
    repeatplay: (from: number, to: number, loop?: any, callback?: () => void) => void;
    from: (from: number, loop?: any, callback?: () => void) => void;
    to: (to: number, loop?: any, callback?: () => void) => void;
    pause: () => void;
...
const KeyAnimation = function (this: KeyAnimationValue, el: any, type: string, imgs: any, options: any) {
    if (!el || !imgs) {
    ...
    // let that = this;
    let imgsLen: number  = null;
    let canvas: HTMLCanvasElement = null;
    let count: number= 0;
    let ctx: CanvasRenderingContext2D = null;
    let timeMac: any = null;
    let state: string = 'stop';
    // 用投机取巧的做法试试，这个值基本代表了无限
    let infinite: number = 1000000000;
    let plusNum: any = null;
    ...
    // 记录上一次播放行为
    let recordFrom: number = 0;
    let recordTo: any = null;
    ...
    init();
} as any as {new (el: any, type: string, imgs: any, options: any) : KeyAnimationValue};
